...
partial class WcrUtilities
{
    ...
    private static bool? _doesWindowsSupportAI;

    public static bool DoesWindowsSupportAI()
    {
        if (_doesWindowsSupportAI.HasValue)
            return _doesWindowsSupportAI.Value;

        OperatingSystem os = Environment.OSVersion;

        if (os.Version.Build < 26200)
        {
            _doesWindowsSupportAI = false;
            return false;
        }

        _doesWindowsSupportAI = true;
        return true;
    }
}
